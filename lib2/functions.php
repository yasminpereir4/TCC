<?php
	...
	function get_users($con){
		$sql = $con->prepare("SELECT * FROM tabela_cadastro ORDER BY codigo DESC");
		$sql->execute();
		$get = $sql->get_result();
		$total = $get->num_rows;
		...
		if($total > 0){
			while($dados = $get->fetch_array()){
				echo "<a href='?pagina=perfil&id={$dados['username']}'>{$dados['nome']}</a><br>";
				verfica_solicitacoes($con, $_SESSION['userLogin'], $dados['username']);
				echo "<br>";
			}
		}
	}
	...
	function get_perfil($con, $perfil){
		$sql = $con->prepare("SELECT * FROM tabela_cadastro WHERE username = ?");
		...
		if($total > 0){
			...
			echo "<h4>{$dados['nome']} <small>{$dados['username']}</small></h4>";
			verfica_solicitacoes($con, $_SESSION['userLogin'], $perfil);
		}
	}

	function get_amigos($con){
		$sql = $con->prepare("SELECT * FROM tabela_cadastro  INNER JOIN amigos");
		$sql->execute();
		$get = $sql->get_result();
		$total = $get->num_rows;
		...
		if($total > 0){
			...
			echo "<h4>{$dados['nome']} <small>{$dados['username']}</small></h4>";
			amigos($con, $_SESSION['userLogin'], $dados['username']);
				}
			}
		}
	}
	...
	function amigos($con, $id_de, $id_para){
		$sql = $con->prepare("SELECT * FROM amigos WHERE (id_de = ? AND id_para = ?) OR (id_para = ? AND id_de = ?)");
		$sql->bind_param("ssss", $id_de, $id_para, $id_de, $id_para);
		$sql->execute();
		$get = $sql->get_result();
